...
import { useProperty } from "@/lib/hooks/useProperty"; // ✅ Add this back
import { supabase } from "@/lib/supabase";
import StandardPageLayout from "@/components/layout/StandardPageLayout";
import StandardCard from "@/components/ui/StandardCard";
import { Building, Plus, Settings, MapPin, Edit, Trash2 } from "lucide-react";
...
export default function PropertySettingsPage() {
  const { user } = useAuth();
  const { currentTenant, userProperties, propertyLoading } = useProperty(); // ✅ Use this instead
  const [properties, setProperties] = useState<Property[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // ✅ FIXED: Role checking - use user_metadata.role
  ...
  // ✅ Use the working role check instead of bypass
  const hasAccess = canManageProperties();

  // ✅ UPDATED: Use properties from useProperty hook instead of separate fetch
  useEffect(() => {
    if (userProperties && userProperties.length > 0) {
      setProperties(userProperties);
      setLoading(false);
    } else if (!propertyLoading) {
      // No properties found and not loading
      setProperties([]);
      setLoading(false);
    }
  ...
  // ✅ Use propertyLoading from hook instead of local loading
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Manage your family's vacation homes and properties
            </p>
          </div>
