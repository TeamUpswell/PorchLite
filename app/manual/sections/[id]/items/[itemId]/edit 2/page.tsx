...
export default function EditItemPage({ params }) {
    ...
    async function fetchData() {
      try {
        ...
        setFormData({
          ...
          media_urls: itemData.media_urls?.length > 0 ? itemData.media_urls : [""],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(`/manual/sections/${params.id}`);
    } catch (error) {
  ...
  return (
    <StandardPageLayout
      title="Edit Item"
      subtitle={`Edit "${item.title}" in "${section.title}"`}
      headerActions={
